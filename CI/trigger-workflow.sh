...
if [[ ! -z $(echo "$post" | grep "HTTP/2 20*" ) ]]; then
    ...
    while [[ $(echo "$run_search" | grep ^'      "name":' | \
        ...
        run_search=$(curl -s -L \
                    ...
                    https://api.github.com/repos/${PRIVATE_ORG}/${PRIVATE_REPO}/actions/workflows/${PRIVATE_WORKFLOW}/runs)
    done
    ...
    while [[ "$run_status" != "completed" ]]; do
        ...
        status_request=$(curl -s -L \
                        ...
                        -H "Authorization: Bearer ${GH_TRIGGER_TOKEN}" \
                        -H "X-GitHub-Api-Version: 2022-11-28" \
                        https://api.github.com/repos/${PRIVATE_ORG}/${PRIVATE_REPO}/actions/runs/${run_id})
        run_status=$(echo "$status_request" | grep ^'  "status":' | cut -d ":" -f2 | \
    ...
    conclusion=$(echo "$status_request" | grep ^'  "conclusion":' | cut -d ":" -f2 | \
                    sed 's#"##g' | sed 's#,##g' | sed 's# ##g')
